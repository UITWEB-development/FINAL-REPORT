<section class="py-5 relative">
    @if ($order_fetch_message && $order->status == 'Unpaid')
    ...
    @else
        <div class="w-full max-w-7xl px-4 md:px-5 lg-6 mx-auto">
            ...
            <div class="mb-10 flex justify-between items-center">
                <h6 class="font-medium text-xl leading-8">{{ $order_messages[$order->status] }}</h6>
                @if ($payment_url)
                    <a class="bg-orange-500 block text-white p-2 rounded-md hover:opacity-70 active:opacity-50" href="{{$payment_url}}">Open Payos checkout</a>
                @endif
            </div>
            <div>
            ...
            <div>
                ...
                <div class=" w-full">
                    ...
                    <div class="flex items-center justify-between mb-6">
                        ...
                        <p class="font-semibold text-xl leading-8 text-gray-900">{{ $order->order_address->address }},
                            {{ $order->order_address->ward->name }},
                            {{ $order->order_address->ward->district->name }},
                            {{ $order->order_address->ward->district->province->name }}</p>
                    </div>
                    <div class="flex items-center justify-between mb-6">
                        <p class="font-normal text-xl leading-8 text-gray-500">Date</p>
                        <p class="font-semibold text-xl leading-8 text-gray-900">
                            {{ \Carbon\Carbon::parse($order->order_date)->format('d/m/Y H:i:s') }}</p>
                    </div>
                    <div class="flex items-center justify-between mb-6">
                        <p class="font-normal text-xl leading-8 text-gray-500">Payment method</p>
                        <p class="font-semibold text-xl leading-8 text-gray-900">
                            {{ Str::upper($order->payment_method) }}</p>
                    </div>

                    @if (auth()->user() && auth()->user()->id === $order->user_id && ($order->status == 'Pending' || $order->status == 'Unpaid'))
                        <div>
                            <button wire:click="cancel" class="block bg-orange-600 px-5 py-3 font-bold text-white hover:opacity-70 active:opacity-50">Cancel order</button>
                        </div>
                    @endif
                </div>
            </div>
